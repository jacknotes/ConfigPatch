...
func (tt *ToolTip) addTool(hwnd win.HWND, track bool) error {
	...
	// PATCH (iisconfig-replace): ToolTips are purely cosmetic. Some Windows
	// sessions cannot create tooltips (TTM_ADDTOOL returns FALSE with no error
	// code, e.g. in restricted/non-interactive desktops). Aborting window
	// creation over a cosmetic feature makes GUI apps unusable there, so the
